...
import JiedanxinxiModel from '../models/JiedanxinxiModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let jiedanshijian = req.query.jiedanshijian
			if (jiedanshijian) {

				if (jiedanshijian.indexOf('%') != -1) {
					where.jiedanshijian = {
						[Op.like]: jiedanshijian
					}
				} else {
					where.jiedanshijian = {
						[Op.eq]: jiedanshijian
			...
			let miaoshuguzhang = req.query.miaoshuguzhang
			if (miaoshuguzhang) {

				if (miaoshuguzhang.indexOf('%') != -1) {
					where.miaoshuguzhang = {
						[Op.like]: miaoshuguzhang
					}
				} else {
					where.miaoshuguzhang = {
						[Op.eq]: miaoshuguzhang
					}
				}
			}
			...
			let result = await JiedanxinxiModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await JiedanxinxiModel.findAll()
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			const userinfo = await JiedanxinxiModel.create(req.body)
	...
	api.post('/update',(req,res,next)=>{
		...
		try {
			...
			await JiedanxinxiModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete',(req,res,next)=>{
		...
		try {
			...
			await JiedanxinxiModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await JiedanxinxiModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM jiedanxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM jiedanxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM jiedanxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM jiedanxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jiedanxinxi WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM jiedanxinxi WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM jiedanxinxi " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("jiedanxinxi" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM jiedanxinxi " + where + " GROUP BY " + xColumnName + " DESC"

...
import WeixiufuwuModel from '../models/WeixiufuwuModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let xiangmufenlei = req.query.xiangmufenlei
			if (xiangmufenlei) {

				if (xiangmufenlei.indexOf('%') != -1) {
					where.xiangmufenlei = {
						[Op.like]: xiangmufenlei
					}
				} else {
					where.xiangmufenlei = {
						[Op.eq]: xiangmufenlei
			...
			let xiangmutupian = req.query.xiangmutupian
			if (xiangmutupian) {

				if (xiangmutupian.indexOf('%') != -1) {
					where.xiangmutupian = {
						[Op.like]: xiangmutupian
					}
				} else {
					where.xiangmutupian = {
						[Op.eq]: xiangmutupian
			...
			let xiangmujiage = req.query.xiangmujiage
			if (xiangmujiage) {

				if (xiangmujiage.indexOf('%') != -1) {
					where.xiangmujiage = {
						[Op.like]: xiangmujiage
					}
				} else {
					where.xiangmujiage = {
						[Op.eq]: xiangmujiage
			...
			let xiangmujieshao = req.query.xiangmujieshao
			if (xiangmujieshao) {

				if (xiangmujieshao.indexOf('%') != -1) {
					where.xiangmujieshao = {
						[Op.like]: xiangmujieshao
					}
				} else {
					where.xiangmujieshao = {
						[Op.eq]: xiangmujieshao
					}
				}
			}
			...
			let result = await WeixiufuwuModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await WeixiufuwuModel.findAll()
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			const userinfo = await WeixiufuwuModel.create(req.body)
	...
	api.post('/update',(req,res,next)=>{
		...
		try {
			...
			await WeixiufuwuModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await WeixiufuwuModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM weixiufuwu WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM weixiufuwu WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM weixiufuwu WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM weixiufuwu WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM weixiufuwu WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM weixiufuwu WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "weixiufuwu"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM weixiufuwu " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("weixiufuwu" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM weixiufuwu " + where + " GROUP BY " + xColumnName + " DESC"

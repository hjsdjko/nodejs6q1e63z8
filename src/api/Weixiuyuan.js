...
import WeixiuyuanModel from '../models/WeixiuyuanModel'
...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await WeixiuyuanModel.findOne({ where: { weixiuzhanghao: req.query.username, mima: pwd } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.weixiuzhanghao,
					role: '维修员',
                    loginUserColumn: 'weixiuzhanghao',
					weixiuzhanghao:userinfo.dataValues.weixiuzhanghao,
					mima:userinfo.dataValues.mima,
					weixiuxingming:userinfo.dataValues.weixiuxingming,
			...
			userinfo.dataValues.role = '维修员'
			userinfo.dataValues.loginUserColumn = 'weixiuzhanghao'
			userinfo.dataValues.tableName = 'weixiuyuan'
	...
	api.post('/register', async (req, res) => {
		...
		try {
			...
			const userinfo = await WeixiuyuanModel.create(req.body)
	...
	api.all('/session', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await WeixiuyuanModel.findOne({ where: { id: req.session.userinfo === undefined ? jwt.decode(req.headers.token).id : req.session.userinfo.id } }))
		} catch(err) {
	...
	api.all('/resetPass', async (req, res) => {
		...
		try {
    		...
    		let pwd = '123456'
			await WeixiuyuanModel.update({
				mima: pwd
			}, {
				where: {
				  weixiuzhanghao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let result = await WeixiuyuanModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await WeixiuyuanModel.findAll()
	...
	api.post('/save',(req,res,next)=>{
		...
		try {
			...
			let weixiuzhanghaocount = await WeixiuyuanModel.count({
				where: {
					weixiuzhanghao: req.body["weixiuzhanghao"]
				}
			})
            if (weixiuzhanghaocount > 0) {
				toRes.session(res, -1, '维修账号已存在')
            }
	...
	api.post('/update',(req,res,next)=>{
		...
		try {
			...
			await WeixiuyuanModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await WeixiuyuanModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM weixiuyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM weixiuyuan WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM weixiuyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM weixiuyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM weixiuyuan WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM weixiuyuan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "weixiuyuan"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM weixiuyuan " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("weixiuyuan" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM weixiuyuan " + where + " GROUP BY " + xColumnName + " DESC"

<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 0px","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0 0 10px","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('fuwupingjia','服务评价统计')" type="success" @click="chartDialog1()">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="fuwutaiduChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var fuwutaiduChart1 = echarts.init(document.getElementById("fuwutaiduChart1"),'macarons');
        this.$http({
            url: "fuwupingjia/typeStat/fuwutaidu/weixiuxingming",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    yAxis1.push(parseFloat((res[i].非常满意)));
                    yAxis2.push(parseFloat((res[i].满意)));
                    yAxis3.push(parseFloat((res[i].一般)));
                    yAxis4.push(parseFloat((res[i].不满意)));
                    pArray.push({
                        name: res[i].fuwutaidu
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                fuwutaiduChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    fuwutaiduChart1.resize();
                };
            }
        });
      })

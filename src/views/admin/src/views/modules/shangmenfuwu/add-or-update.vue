	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="date" v-if="type!='info'" label="上门时间" prop="shangmenshijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.shangmenshijian" 
						type="datetime"
						:readonly="ro.shangmenshijian"
						placeholder="上门时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="input" v-else-if="ruleForm.shangmenshijian" label="上门时间" prop="shangmenshijian">
					<el-input v-model="ruleForm.shangmenshijian" placeholder="上门时间" readonly></el-input>
				</el-form-item>
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shangmenfuwu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shangmenfuwuCrossAddOrUpdateFlag = false;
											this.parent.search();

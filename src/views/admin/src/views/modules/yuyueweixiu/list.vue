		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 0px","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0 0 10px","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yuyueweixiu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-kaitongfuwu" :style='{"margin":"0 2px","fontSize":"inherit","color":"#fff","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yuyueweixiu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-guanbi1" :style='{"margin":"0 2px","fontSize":"inherit","color":"#fff","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuyueweixiu','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('yuyueweixiu','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuyueweixiu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-dagou" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueweixiu','接单')" @click="jiedanxinxiCrossAddOrUpdateHandler(scope.row,'cross','否','是','zhuangtai','已被接单','已接单,未接单'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								接单
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuyueweixiu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yuyueweixiu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
	...
	export default {
		...
		methods: {
			...
			jiedanxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','yuyueweixiu');
			...
			payHandler(row){
				this.$storage.set('paytable','yuyueweixiu');

...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="紧急程度" prop="jinjichengdu">
            <el-input v-model="ruleForm.jinjichengdu" 
                placeholder="紧急程度" clearable :disabled=" false  ||ro.jinjichengdu"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="上门时间" prop="shangmenshijian">
              <el-date-picker
				  :disabled=" false  ||ro.shangmenshijian"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="地址" prop="dizhi">
            <el-input v-model="ruleForm.dizhi" 
                placeholder="地址" clearable :disabled=" false  ||ro.dizhi"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="维修账号" prop="weixiuzhanghao">
            <el-input v-model="ruleForm.weixiuzhanghao" 
                placeholder="维修账号" clearable :disabled=" false  ||ro.weixiuzhanghao"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="维修姓名" prop="weixiuxingming">
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.shangmenshijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`shangmenfuwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('shangmenfuwu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`shangmenfuwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
